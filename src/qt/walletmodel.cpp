...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 AGR.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzAGRReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzAGRReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzAGRReset.connect(boost::bind(NotifyzAGRReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzAGRReset.disconnect(boost::bind(NotifyzAGRReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zagrTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
