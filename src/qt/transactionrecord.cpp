...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZAGRModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zagrTracker->HasMintTx(hash))) {
            //zAGR stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZAGR;
...
bool IsZAGRType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZAGR:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZAGR || type == TransactionRecord::MNReward) {
